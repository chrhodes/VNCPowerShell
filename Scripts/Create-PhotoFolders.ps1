...
# <TODO: Add script level parameters>
    [int16] $StartPageNumber   = 1,
    [int16] $EndPageNumber     = 30,
...
function Func1()
{
    $message = "Creating PhotoPages"
    LogMessage $message "Func1" "Info"

    for ($pageNumber = $StartPageNumber ;  $pageNumber -le $EndPageNumber ; $pageNumber++ )
    {
        LogMessage "Creating Page $pageNumber" "Info"
        New-Item -Path . -Name Page$pageNumber -ItemType Directory
    }
}
