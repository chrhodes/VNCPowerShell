...
{ 
 ...
 # <TODO: Add code, functional calls here to do something cool>   
	...
	if ($DebugConfig) { SwitchConfiguration "-Release" "-Dev" }

	if ($ReleaseConfig) { SwitchConfiguration "-Dev" "-Release" }
...
{
	Foreach ($app in $clientApplications)
	{
		if (VerifyFilesExist "$app.exe.config" "$app$to.exe.config" )
		{
			LogMessage "  --> Moving $app.exe.config to $app$from.exe.config" "SwitchConfiguration" "Trace"

			Move-Item "$app.exe.config" "$app$from.exe.config" -Force

			LogMessage "  <-- Moving $app$to.exe.config to $app.exe.config" "SwitchConfiguration" "Trace"

			Move-Item "$app$to.exe.config" "$app.exe.config" -Force
		}
	}
}
